...
bIdentifyRanges = 1;
nHeightOfBrainSlice=80 ;   % in pixels
...
nFinalFilter = 6;                    %Filter for pdLongTrace and pdLongMask
...
for i =1:length(nFolder);
    ...
    strFolderSaveData = [strData,num2str(nFolder(i)),'\BPPh6af20dBM2Corr3rd\'];
...
%             subplot(313), plot(BulkMotionCorrPhase(nSelectedLayer,11:2048-11,nStartingFrame)); title('BulkMotionCorrPhase');axis tight;
            ...
            %2nd correction
%             figure();
            BulkCorrectionFactor2nd=GenerateBulkmotionCorrFactor2ndTime(pdPeakSNR,dSNRThreshold,BulkMotionCorrPhase,nPhaseDiff,nPhaseFilterLength);
%             subplot(211), plot(BulkCorrectionFactor(nSelectedLayer,11:2048-11,nStartingFrame)); title('BulkCorrectionFactor [2nd]');axis tight;
            BulkMotionCorrPhase=BulkMotionCorrPhase-BulkCorrectionFactor2nd;
%             subplot(212), plot(BulkCorrectionFactor2nd(nSelectedLayer,11:2048-11,nStartingFrame)); title('BulkMotionCorrPhase [2nd]');axis tight;
          

          %3rd correction
%             figure();
            BulkCorrectionFactor3rd=GenerateBulkmotionCorrFactor2ndTime(pdPeakSNR,dSNRThreshold,BulkMotionCorrPhase,nPhaseDiff,nPhaseFilterLength);
%             subplot(211), plot(BulkCorrectionFactor(nSelectedLayer,11:2048-11,nStartingFrame)); title('BulkCorrectionFactor [2nd]');axis tight;
            BulkMotionCorrPhase=BulkMotionCorrPhase-BulkCorrectionFactor3rd;
%             subplot(212), plot(BulkCorrectionFactor2nd(nSelectedLayer,11:2048-11,nStartingFrame)); title('BulkMotionCorrPhase [2nd]');axis tight;
        ...
        % first filter: avg filter
        [pdLongTraceUnFiltered] =GenerateLongTraceAndAvfFilter( BulkMotionCorrPhase,nFinalFilter,nStartingFrame,bInitialTest);
        
        pdLongTraceUnFiltered1=reshape(BulkMotionCorrPhase, size(BulkMotionCorrPhase,1), size(BulkMotionCorrPhase,2)*size(BulkMotionCorrPhase,3));
        
%         r=50;
%         figure,
%         subplot(311),plot( pdLongTraceUnFiltered1(r,:));axis tight;title('unfiltered');
...
%         
%         figure,
%         subplot(311),plot( pdLongTraceUnFiltered(r,:));axis tight;title('After avg filter');
...
%         Phase2=pdLongTraceUnFiltered(r,:);
...
%         
%         Phase3=pdLongTraceAllFiltered(r,:);
%         [f,Y]=FindFreqRange(Phase3);
%         
%         
%         figure,
%         subplot(311),plot( pdLongTraceAllFiltered(r,:));axis tight;title('After BP filter');
%         subplot(312),plot( pdLongTraceAllFiltered(r+1,:));axis tight;
%         subplot(313),plot( pdLongTraceAllFiltered(r,:)- pdLongTraceUnFiltered(r+1,:));axis tight;
%         
%         keyboard;
